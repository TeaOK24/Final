...
const { america_cuisine } = base;


class america_cuisine_controller {
    async create(req, res) {
        try {
            const { name, ingredients, photo, manual, estimation } = req.body;
            const check = await america_cuisine.findOne({ where: { name } })
            if (check) { return res.json("неполучилось добавить") }
            const food = await america_cuisine.create({ name, ingredients, photo, manual, estimation })
            res.json(food)
        }
        catch (e) {
            res.json(e)
    ...
    async find(req, res) {
        try {
            ...
            if (!name) {
                const food = await america_cuisine.findAndCountAll()
                return res.json(food)
            }
            const food = await america_cuisine.findAndCountAll({ where: { name } })
    ...
    async delete(req, res) {
        try {
            const { id } = req.params
            const food = await america_cuisine.findByPk(id)
            if (!food) {
