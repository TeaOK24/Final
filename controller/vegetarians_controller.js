...
const { vegetarian } = base;


class vegetarians_controller {
    ...
    async find(req, res) {
        try {
            ...
            if (!name) {
                const food = await vegetarian.findAndCountAll()
                return res.json(food)
            }
            const food = await vegetarian.findAndCountAll({ where: { name } })
    ...
    async delete(req, res) {
        try {
            const { id } = req.params
            const food = await vegetarian.findByPk(id)
            if (!food) {
