...
const { mexico_cuisine } = base;


class mexico_cuisine_controller {
    async create(req, res) {
        try {
            const { name, ingredients, photo, manual, estimation } = req.body;
            const check = await mexico_cuisine.findOne({ where: { name } })
            if (check) { return res.json("неполучилось добавить") }
            const food = await mexico_cuisine.create({ name, ingredients, photo, manual, estimation })
            res.json(food)
        }
        catch (e) {
            res.json(e)
    ...
    async find(req, res) {
        try {
            ...
            if (!name) {
                const food = await mexico_cuisine.findAndCountAll()
                return res.json(food)
            }
            const food = await mexico_cuisine.findAndCountAll({ where: { name } })
    ...
    async delete(req, res) {
        try {
            const { id } = req.params
            const food = await mexico_cuisine.findByPk(id)
            if (!food) {
